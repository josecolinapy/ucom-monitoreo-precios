...
def generar_planilla():
    ...
    n = len(df7.index)
    x = np.arange(n)
    plt.bar(x, df7['1'], width=width, label='Proveedor 1')
    plt.xticks(x, df7.index)
    plt.legend(loc='best')
    plt.show()
    plt.savefig('Proveedor 1.png')

    # ------------------Proveedor 2--------------------------------------------------------------
    n = len(df7.index)
    x = np.arange(n)
    plt.bar(x, df7['2'], width=width, label='Proveedor 2')
    plt.xticks(x, df7.index)
    plt.legend(loc='best')
    plt.show()
    plt.savefig('Proveedor 2.png')

    # ------------------Proveedor 3--------------------------------------------------------------
    n = len(df7.index)
    x = np.arange(n)
    plt.bar(x, df7['3'], width=width, label='Proveedor 3')
    plt.xticks(x, df7.index)
    plt.legend(loc='best')
    plt.show()
    plt.savefig('Proveedor 3.png')

    # --------------------------------------------------------------------------------
    plt.bar(df7.index, df7['1'] + df7['2'] + df7['3'], label='Proveedor 1')
    plt.bar(df7.index, df7['2'] + df7['3'], label='Proveedor 2')
    plt.bar(df7.index, df7['3'], label='Proveedor 3')
    plt.legend(loc='best')
    plt.show()
    plt.savefig('Resumen Proveedor.png')

    # --------------------------------------------------------------------------------
    n = len(df7.index)
    x = np.arange(n)
    width = 0.25
    plt.bar(x - width, df7['1'], width=width, label='Proveedor 1')
    plt.bar(x, df7['2'], width=width, label='Proveedor 2')
    plt.bar(x + width, df7['3'], width=width, label='Proveedor 3')
    plt.xticks(x, df7.index)
    plt.legend(loc='best')
    plt.show()
    plt.savefig('Resumen Proveedor 2.png')
    ...
    fig, ax = plt.subplots()
    # Generamos las barras para el conjunto de Proveedor1
    rects1 = ax.bar(x - width, df7['1'], width, label='Proveedor1')
    # Generamos las barras para el conjunto de Proveedor1
    rects2 = ax.bar(x, df7['2'], width, label='Proveedor2')
    # Generamos las barras para el conjunto de Proveedor3
    rects3 = ax.bar(x + width, df7['3'], width, label='Proveedor3')
    ...
    df.to_excel('Reporte_Proveedors_.xlsx')
    df7.to_excel('Reporte_Proveedors_canasta.xlsx')
...
if __name__ == "__main__":
    ...
    tipo_ejecucion = read_config(config, 'environment', 'tipo_ejecucion')
    data = []
    url_proveedor_2 = config.get("environment", "url_proveedor_2")
    url_proveedor_3 = config.get("environment", "url_proveedor_3")
    data.extend(obtener_datos.proceso_obtener_datos_proveedor(proveedor='1', tipo=tipo_ejecucion))
    data.extend(obtener_datos.proceso_obtener_datos_proveedor(proveedor='2', tipo=tipo_ejecucion, url=url_proveedor_2))
    data.extend(obtener_datos.proceso_obtener_datos_proveedor(proveedor='3', tipo=tipo_ejecucion, url=url_proveedor_3))
    # recorremos la data
